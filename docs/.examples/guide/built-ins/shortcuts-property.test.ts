...
it('built-in shortcuts property output matches engine', async ({ expect }) => {
	const usage = await readExampleFile(new URL('./shortcuts-property.pikainput.ts', import.meta.url))
	const css = await renderExampleCSS({
		config,
		usageCode: usage,
	})
	await expect(css).toMatchFileSnapshot('./shortcuts-property.pikaoutput.css')
})
