...
it('icons-usage-vue output matches engine', async ({ expect }) => {
	const usage = await readExampleFile(new URL('./icons-usage-vue.pikainput.vue', import.meta.url))
	...
	await expect(css).toMatchFileSnapshot('./icons-usage-vue.pikaoutput.css')
})
