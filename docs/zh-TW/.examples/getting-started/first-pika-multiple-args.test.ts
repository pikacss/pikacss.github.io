...
it('first-pika-multiple-args output matches engine', async ({ expect }) => {
	const usage = await readExampleFile(new URL('./first-pika-multiple-args.pikainput.vue', import.meta.url))
	const css = await renderExampleCSS({
		config,
		usageCode: usage,
	})
	await expect(css).toMatchFileSnapshot('./first-pika-multiple-args.pikaoutput.css')
})
