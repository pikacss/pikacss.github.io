...
| 需求 | Hook | 為什麼這是侵入性最低的選項 |
| --- | --- | --- |
| 在 defaults 套用前修改 user config | `configureRawConfig` | 在 resolution 前一次塑形輸入 config |
| 在 config resolved 後做出反應 | `configureResolvedConfig` | 可以根據正規化完成的 config 行動，而不改寫抽出的 payload |
| 透過公開 API 註冊 engine 行為 | `configureEngine` | additive engine APIs 比 payload rewrites 更容易組合 |
| 直接改寫 selector chains | `transformSelectors` | 只有在 selector registration 無法表達變更時才需要 |
| 改寫抽出的 style items | `transformStyleItems` | 只有在你必須替換抽出的 item payload 時才需要 |
| 改寫巢狀 style definitions | `transformStyleDefinitions` | 只有在 definition tree 本身必須被直接改寫時才需要 |
| 只觀察 engine events | sync notification hooks | 用於 logs、副作用或診斷資訊 |
...
4. 抽出的 payload 在 pipeline 中流動時觸發的 async transform hooks
5. 下游事件發出後的 sync notification hooks

這個順序也說明了為什麼一般建議是先從 `configureEngine` 開始，只有在你能精確說出少了哪個能力時，才往更早或更深的 hooks 走。
