...
把 style object 巢入 selector key 之下是在原始碼中宣告的，而不是在 runtime 計算出來的。當 key 是以下其中一種時，它就與 build-time model 完全相容：

- 已登記的 selector 名稱，例如 `hover`、`theme-dark` 或 `screen-md`
- 直接使用的 at-rule，例如 `@media (min-width: 768px)`

對 pseudo states 與可重用的 context 規則，請優先使用已登記的 selector 名稱，讓 engine 輸出 flat atomic rules，而不是零散的 nesting output。

完整 API（包含 `$` placeholder、default selector 的心智模型，以及像 `@media` 這種 wrapper 條件如何展開）請參閱 [Selectors](/zh-TW/guide/core-features/selectors)。
