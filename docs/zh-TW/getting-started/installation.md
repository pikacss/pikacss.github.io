...
PikaCSS 會自動尋找名為 `pika.config.{js,ts,mjs,mts,cjs,cts}` 的 config files。Zero-config 適合第一次驗證，但大多數真實專案一旦需要 selectors、shortcuts、variables、plugins 或一致的 layer 控制，就應該儘快補上 config file。
