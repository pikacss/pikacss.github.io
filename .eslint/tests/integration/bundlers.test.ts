...
describe('vite Integration', () => {
	...
	it('should compile valid pika() examples', { timeout: 300000 }, async () => {
...
export default defineConfig({
  ...
  build: { rollupOptions: { input: './src/valid.ts' } }
	...
	it('should fail on runtime-dynamic pika() calls', { timeout: 300000 }, async () => {
...
describe('nuxt Integration', () => {
	...
	beforeEach(async () => {
		...
		await execa('pnpm', ['install', '--no-frozen-lockfile'], { cwd: testDir })
	}, 300000) // 300s timeout for Nuxt dependencies (large dependency tree)

	afterEach(async () => {
		await rm(testDir, { recursive: true, force: true })
	})

	it('should build Nuxt app with valid pika()', { timeout: 360000 }, async () => {
...
describe('webpack Integration', () => {
	...
	it('should compile valid pika() with Webpack', { timeout: 300000 }, async () => {
		const result = await execa('pnpm', ['build'], { cwd: testDir, reject: false })
